...
def _debug_image(name: str, img: np.ndarray) -> None:
    ...
    if img.shape[-1] != 3:
        print(
            f"  [DBG] WARNING: expected 3-channel image for '{name}', got {img.shape[-1]} channels"
        )
...
def _parse_observations(observations, metadata, debug: bool = False):
    ...
    qpos = last_observation["position"].values.to_numpy(zero_copy_only=False).reshape(pos_shape)
...
def _infer(
    ...
    if debug:
        ...
        for k, v in last_chunk.items():
            arr = np.asarray(v)
            print(
                f"  [DBG] chunk['{k}']: shape={arr.shape} dtype={arr.dtype} min={arr.min():.4f} max={arr.max():.4f}"
            )
...
def _process_request(io, adapter, infer_hz=10, debug: bool = False):
    ...
    for request_json in io:
        if n_received == 0:
            print(
                f"First raw line received ({len(request_json)} bytes): {request_json[:120]!r}",
                flush=True,
            )
        ...
        if elapsed < INTERVAL_NS:
            ...
            if debug:
                print(
                    f"[DBG] throttled (elapsed={elapsed / 1e6:.1f}ms < {INTERVAL_NS / 1e6:.1f}ms)"
                )
        ...
        with pa.OSFile(request["data_path"], "rb") as source:
            with pa.ipc.open_file(source) as reader:
                ...
                # task_prompt is a pyarrow StringScalar (or null before the UI
                # sends one); convert to a plain str so the GR00T processor can
                # call .lower() on it. Fall back to "" to avoid crashing during
                # the startup window when no prompt has arrived yet.
                language_instruction = observations[-1]["task_prompt"].as_py() or ""
                actions = _infer(
                    adapter, *obs, language_instruction=language_instruction, debug=debug
                )
                if debug:
                    print(
                        f"[DBG] sending {len(actions['positions'])} position steps, cutoff_hz={actions['cutoff_hz']}"
                    )
                io.write(json.dumps(actions) + "\n")
                io.flush()

    print(
        f"Connection closed (received={n_received} inferred={n_inferred} throttled={n_throttled})",
        flush=True,
    )
...
class OpenArmGR00TAdapter:
    ...
    def split_qpos(self, qpos: np.ndarray) -> dict[str, np.ndarray]:
        ...
        if self.bimanual:
            ...
            if qpos.shape[0] != 8:
                raise ValueError(f"Expected 8-DoF qpos for single-arm, got {qpos.shape}")
            return {
                "right_arm": qpos[0:7],
                "right_gripper": qpos[7:8],
            }
...
def main():
    ...
    parser.add_argument("--local-server", action="store_true", help="Run as a local policy server.")
    parser.add_argument("--infer-hz", default=10.0, type=float, help="Inference frequency in Hz")
    parser.add_argument(
        "--debug",
        action="store_true",
            ...
            with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as sock:
                ...
                while True:
                    ...
                    with conn, conn.makefile("rw") as io:
                        _process_request(io, adapter, args.infer_hz, debug=args.debug)
        finally:
